...
CO_Status CANOpen_sendPDO(uint8_t nodeId, uint8_t channel, CO_PDOStruct* pdo_struct){
  ...
  while(1){
    ...
    case 8 : tmp64 |= *(uint64_t*)pdo_struct->data[i]; break;
    }
...
void CANOpen_mappingPDO_uint64(CO_PDOStruct* pdo_struct, uint64_t* data){
  CANOpen_mappingPDO(pdo_struct, data, 64);
}
